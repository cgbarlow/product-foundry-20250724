...
class MockStoryEngine {
    ...
    this.chapters = [
      {
        ...
        scenes: [
          ...
          },
          {
            ...
            choices: [
              { id: 'continue', text: 'Continue adventure', next: 'ch1_start' }
            ],
      ...
      },
      {
        ...
        scenes: [
          ...
          },
          {
            id: 'ch1_explore',
            title: 'Exploration',
            content: 'You explore the environment.',
            choices: [],
            triggers: [],
            consequences: []
          },
          {
            id: 'ch1_talk',
            title: 'Conversation',
            content: 'You talk more with Ravi.',
            choices: [],
            triggers: [],
            consequences: []
          }
  ...
  makeChoice(choiceId) {
    ...
    if (nextScene) {
      ...
      // Apply consequences from the destination scene
      this.applyConsequences(nextScene.scene.consequences)
      
      // Set story flags from the destination scene
      nextScene.scene.triggers.forEach(trigger => {
  ...
  getBranchingPaths() {
    ...
    const explorePath = (sceneId, path = []) => {
      // Check for cycles in current path to prevent infinite loops
      if (path.includes(sceneId)) {
        paths.push([...path, sceneId])
        return
      }
      ...
      if (scene.choices.length === 0) {
        ...
        // Continue exploring each choice
